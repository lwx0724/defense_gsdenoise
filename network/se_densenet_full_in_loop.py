...
def se_densenet121(pretrained=False, is_strict=False, **kwargs):
    ...
    model = SEDenseNet(num_init_features=64, growth_rate=32, block_config=(6, 12, 24, 16),
                     **kwargs)
    if pretrained:
        print('pretrain_dir:', cfg.pretrain)
        model.load_state_dict(torch.load(cfg.pretrain)['model_state_dict'])
        # state_dict = torch.load(cfg.pretrain)['model_state_dict']
...
class SEDenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16),
                 num_init_features=64, bn_size=4, drop_rate=0, num_classes=110):
